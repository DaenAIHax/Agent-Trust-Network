...
# ═══════════════════════════════════════════════════════════════════════════════
# Cullis Demo — ElectroStore (Buyer) Bootstrap
#
...
#   ORG_ID        — org identifier          (default: electrostore)
#   AGENT_ID      — agent identifier        (default: electrostore::procurement)
#   VAULT_ADDR    — Vault address           (default: http://127.0.0.1:8200)
#   VAULT_TOKEN   — Vault token             (default: demo-buyer-token)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ── Defaults ─────────────────────────────────────────────────────────────────
ORG_ID="${ORG_ID:-electrostore}"
AGENT_ID="${AGENT_ID:-electrostore::procurement}"
DISPLAY_NAME="${DISPLAY_NAME:-ElectroStore S.r.l.}"
AGENT_DISPLAY_NAME="${AGENT_DISPLAY_NAME:-Procurement Agent}"
CAPABILITIES="${CAPABILITIES:-order.read,order.write}"

VAULT_ADDR="${VAULT_ADDR:-http://127.0.0.1:8200}"
VAULT_TOKEN="${VAULT_TOKEN:-demo-buyer-token}"
...
echo -e "\n${BOLD}Cullis Demo — ${DISPLAY_NAME} (${ORG_ID})${RESET}"
...
curl -sf "${VAULT_ADDR}/v1/sys/health" > /dev/null 2>&1 || die "Vault not reachable at ${VAULT_ADDR}. Run: docker compose up -d vault"
ok "Vault is ready at ${VAULT_ADDR}"

# Enable KV v2 (idempotent — already mounted in dev mode)
curl -sf -X POST "${VAULT_ADDR}/v1/sys/mounts/secret" \
  -H "X-Vault-Token: ${VAULT_TOKEN}" \
  -d '{"type":"kv","options":{"version":"2"}}' 2>/dev/null || true

# ═════════════════════════════════════════════════════════════════════════════
# 2. Generate Org CA (RSA 4096)
# ═════════════════════════════════════════════════════════════════════════════
step "Generating Org CA certificate"

openssl genrsa -out "${WORK_DIR}/ca-key.pem" 4096 2>/dev/null

openssl req -new -x509 -key "${WORK_DIR}/ca-key.pem" \
  -out "${WORK_DIR}/ca-cert.pem" -days 365 \
  -subj "/O=${ORG_ID}/CN=${ORG_ID} CA" 2>/dev/null

ok "Org CA generated (RSA 4096, CN=${ORG_ID} CA)"
...
openssl genrsa -out "${WORK_DIR}/agent-key.pem" 2048 2>/dev/null
...
echo -e "  2. Copy CA cert from Vault UI → broker dashboard Settings"
echo -e "     Vault UI: ${VAULT_ADDR}  (token: ${VAULT_TOKEN})"
echo -e "     Path: secret/ → org-ca → ca_cert_pem"
echo -e "  3. Start agent: docker compose up buyer-app"
echo ""
